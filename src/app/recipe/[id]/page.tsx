...
export default async function RecipePage({
  ...
  return (
    <div className="min-h-full">
      {/* The top bar is `position: fixed` (see RecipeTopBar), so this padding
          is the space it would have taken in the flow — ~57px of bar plus a
          little air. It came down from `pt-24` when the bar lost its pill:
          without the capsule's padding and shadow the bar is shorter, and
          leaving the old reserve behind would have opened a cream gap above
          every cover image. */}
      <main className="mx-auto max-w-2xl px-5 pb-32 pt-16">
        ...
        <div dir={bodyDir}>
          ...
          {/* One line, one voice. This row used to be three typefaces and two
            colours — servings in the body font, the cooking time in mono pink,
            the source link in pink again — which read as three separate
            claims of importance under a title that had already made the only
            one worth making. They're all the same kind of fact about the
            recipe, so they get the same size, weight and grey, and the eye
            can take the row in as a row.
          ...
          <div className="mt-3 flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-sm font-semibold text-muted">
            ...
            {recipe.cuisine && (
              <span dir={dirFor(recipe.cuisine)}>{recipe.cuisine}</span>
            )}
            {recipe.source_url && (
              // Grey like its neighbours; the underline on hover is what says
              // it's a link, along with the arrow already in the label.
              <a
          ...
          {stps.length > 0 &&
            (() => {
              ...
              const classified = whole.map((s) => ({
                ...s,
                kind: classifyStepKind(s.text),
              }));
              const instructions = classified.filter(
                (s) => s.kind === "instruction",
              );
